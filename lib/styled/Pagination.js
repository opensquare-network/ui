...
export default function Pagination({
  ...
  const makeEllipsisItem = (targetPage) => {
    const offsetPage = Math.abs(page - targetPage);
    return offsetPage > 1 && <Ellipsis key={offsetPage}>...</Ellipsis>;
  };
  const items = useMemo(() => {
    // always display items
    const firstPageItem = makePageItem(1);
    const lastPageItem = makePageItem(totalPages);
    ...
    const prevEllipsisItem = makeEllipsisItem(1);
    const nextEllipsisItem = makeEllipsisItem(totalPages);
